...
const PRODUCT_IDS = ['star_like_1', 'star_like_2'];

export default function GetStarLike() {
  const {
    connected,
    products,
    requestPurchase,
  ...
  try {
    ...
    const response = await fetch(
      ...
      'http://103.150.227.223:2388/merchant/purchase',
      {
        method: 'POST',
        headers: {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={true}>
      ...
      <View style={styles.hargaCard}>
        <Text style={styles.card2Title}>Select Your Plan</Text>
      ...
      <View style={styles.buttonContainer}>
        ...
        >
          <Text style={styles.buyButtonText}>
            {selectedProduct 
              ? `Buy ${selectedProduct.title?.split(' ')[0] || 'Premium'} - ${selectedProduct.displayPrice}`
